            ...
            <div class="col-12">
                <div class="card card-info">
                    <div class="card-header p-0 pt-1">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill" href="#custom-tabs-one-home" role="tab" aria-controls="custom-tabs-one-home" aria-selected="true">Tokocabang SLA Summary</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" id="custom-tabs-one-profile-tab" data-toggle="pill" href="#custom-tabs-one-profile" role="tab" aria-controls="custom-tabs-one-profile" aria-selected="false">Tokocabang SLA In 7 Days</a>
                        </li>
                        </ul>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="tab-pane fade show active" id="custom-tabs-one-home" role="tabpanel" aria-labelledby="custom-tabs-one-home-tab">
                            <div class="col-md-14">
                                ...
                                <div class="card-header">
                                    ...
                                    <button type="button" id="reload" onclick="tokocabangSummary()" class="btn btn-tool" data-card-widget="card-refresh" data-source="<?php echo base_url() ?>admin/outbound/C_outbound_tokocabang" data-source-selector="#card-refresh-content" data-load-on-init="false">
                                    ...
                                    <div class="card-body">
                                        <table id="purchase" id="packinglist-grid" class="table table-bordered table-striped table-sm" style="font-size:13px">
                                            ...
                                            <tbody  class="nilaislatokocabang">
                                                <tr class="odd">
                            ...
                            <div class="col-md-14">
                                <div class="card card-info shadow-sm">
                                    <div class="card-header">
                                        ...
                                        <div class="card-tools">
                                        <button type="button" id="reload2" onclick="tokocabangPerDay()" class="btn btn-tool" data-card-widget="card-refresh" data-source="<?php echo base_url() ?>admin/outbound/C_outbound_tokocabang" data-source-selector="#card-refresh-content" data-load-on-init="false">
                                            <i class="fas fa-sync-alt"></i>
                                        </button>
                                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                                            <i class="fas fa-minus"></i>
                                    ...
                                    <div class="card-body">
                                        <table id="purchase" id="packinglist-grid" class="table table-bordered table-striped table-sm" style="font-size:13px">
                                            ...
                                            <tbody class="nilaisladaytokocabang">
                                            <tr>
...
<script type="text/javascript">
    ...
    function tokocabangSummary() 
    {
        var date = new Date();
        var chart;
        var reload = $('#reload').val();
        var uri = '<?php echo base_url(); ?>admin/outbound/C_outbound_tokocabang/getAllslaTokocabang';
    
        axios.post(uri)
        .then(response => {
            var result = response.data;
            $(".nilaislatokocabang").find('tr').remove();
            var nilaislatokocabang = $(".nilaislatokocabang");
            $.each(result.nilaislatokocabang, function(key, item) 
            {
                var inti;
                inti = '<tr class="odd">';
                inti += '<td style="text-align:center">' + item.parent + '</td>';
                ...
                inti += '</tr>';
                nilaislatokocabang.append(inti);
            });
    ...
    function tokocabangPerDay() 
    {
        var date = new Date();
        var chart;
        var reload = $('#reload2').val();
        var uri = '<?php echo base_url(); ?>admin/outbound/C_outbound_tokocabang/getAllslaDayTokocabang';
    
        axios.post(uri)
        .then(response => {
            var result = response.data;
            $(".nilaisladaytokocabang").find('tr').remove();
            var nilaisladaytokocabang = $(".nilaisladaytokocabang");
            $.each(result.nilaisladaytokocabang, function(key, item) 
            {
                var inti;
                inti = '<tr class="odd">';
                inti += '<td style="text-align:center">' + item.parent + '</td>';
                inti += '<td style="text-align:center">' + item.location_name + '</td>';
                inti += '<td style="text-align:center">' + item.packed_1 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_1 + '</td>';
                inti += '<td style="text-align:center">' + item.packed_2 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_2 + '</td>';
                inti += '<td style="text-align:center">' + item.packed_3 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_3 + '</td>';
                inti += '<td style="text-align:center">' + item.packed_4 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_4 + '</td>';
                inti += '<td style="text-align:center">' + item.packed_5 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_5 + '</td>';
                inti += '<td style="text-align:center">' + item.packed_6 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_6 + '</td>';
                inti += '<td style="text-align:center">' + item.packed_7 + '</td>';
                inti += '<td style="text-align:center">' + item.rts_7 + '</td>';
                inti += '</tr>';
                nilaisladaytokocabang.append(inti);
            });
            // console.log(result);
        })
        .catch(errors => {
