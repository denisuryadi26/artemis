...
class C_serialnumber extends CI_Controller {
    ...
    public function export()
    {
        ...
        if(!empty($all_client))
                $file_name = "serialnumber_alllocation_".date("Ymd")."_".date("His");
            else
                $file_name = "serialnumber_".strtolower(str_replace(' ', '_', $program_name))."_".date("Ymd")."_".date("His");
        ...
        $serialnumber       = " SELECT
                            ...
                            FROM serialnumber
                            WHERE 1=1 
                            AND item_managed_name = 'Serial Number' 
                            $location
                            $client
                            $p_code
                            $i_code
        ...
        // $data['serialnumberinfo']  =  $this->db->query($serialnumber)->result();
        // $data['all_client']         = $this->input->post('all_client');
        // $data['all_location']        = $this->input->post('all_location');
        
        
        // $this->load->view('admin/reportinglist/export/exportserialnumber', $data);
        ...
        // get data 
        $serialnumberdownload     =  $this->db->query($serialnumber)->result_array();
        ...
        fputcsv($file, $header);
        foreach ($serialnumberdownload as $row){ 
            fputcsv($file,$row);
        }
        fclose($file); 
        exit;
        // $serialnumberinfo  =  $this->db->query($serialnumber)->result();
        ...
        // ->setCellValue('Q'.$i, $data->putaway_date);
        // $i++;
        // }
        ...
        // // Rename worksheet
        // $spreadsheet->getActiveSheet()->setTitle('Serial Number '.date('Ymd'));
