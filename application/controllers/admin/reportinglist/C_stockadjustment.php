...
class C_stockadjustment extends CI_Controller {
    ...
    public function export()
    {
        ...
        if (!empty($start_date_field) || !empty($end_date_field) )
        {
            ...
            {
                $start_date = "AND DATE(adjustment_date) = '$start_date_field' ";
                $end_date   = "";
            }
            else
            {
                if (!empty($start_date_field)){
                    $start_date = "AND adjustment_date >= '$start_date_field'";
                }
                else{
                    $start_date = "";
                }
                if (!empty($end_date_field)){
                    $end_date = "AND adjustment_date <= DATE(TO_DATE('$end_date_field', 'YYYY-MM-DD') + interval '1 day')";
                }
                else{
                    $end_date = "";
                }
