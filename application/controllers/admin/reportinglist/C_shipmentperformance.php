...
class C_shipmentperformance extends CI_Controller {
	...
	//Halaman C_shipmentperformance
	public function index()
	{
		$data['navlocation'] = $this->M_nav->getNav();
		$data['title'] = 'Shipment Performance';
    ...
    public function export()
    {
        ...
        if(($start_date != "" || $start_date != NULL) && ($end_date != "" || $end_date != NULL))
		{
            ...
            if($interval->days <= 31)
            {
                ...
                if(!empty($all_client))
                        $file_name = "shipment_performance_all_client_".date("Ymd")."_".date("His");
                    else
                        $file_name = "shipment_performance_".strtolower(str_replace(' ', '_', $program_name))."_".date("Ymd")."_".date("His");
                ...
                $shipmentperformance       = " SELECT
                                    ...
                                    backorder_date,
                                    pending_by,
                                    TO_CHAR(pending_date, 'mm/dd/YYYY HH24:MI') AS pending_date,
                                    ready_to_picked_by,
                                    TO_CHAR(ready_to_pick_date, 'mm/dd/YYYY HH24:MI') AS ready_to_pick_date,
                                    picked_by,
                                    TO_CHAR(picked_date, 'mm/dd/YYYY HH24:MI') AS picked_date,
                                    ready_to_shipped_by,
                                    TO_CHAR(ready_to_shipped_date, 'mm/dd/YYYY HH24:MI') AS ready_to_shipped_date,
                                    shipped_by,
                                    TO_CHAR(shipped_date, 'mm/dd/YYYY HH24:MI') AS shipped_date,
                                    delivered_by,
                                    TO_CHAR(delivered_date, 'mm/dd/YYYY HH24:MI') AS delivered_date,
                                    return_by,
                                    TO_CHAR(return_date, 'mm/dd/YYYY HH24:MI') AS return_date
                                FROM shipmentperformance
                                WHERE 1=1 
                                $location
                                $client
                                $start_date
                                $end_date";
                // echo $shipmentperformance;die;
                // file name 
                ...
                // get data 
                $shipmentperformancedownload     =  $this->db->query($shipmentperformance)->result_array();
                ...
                fputcsv($file, $header);
                foreach ($shipmentperformancedownload as $row){ 
                    fputcsv($file,$row);
                }
                fclose($file); 
                exit;
            ...
            else
            {
                ...
                redirect(array('admin/reportinglist/C_shipmentperformance'));
            }
        }
        else
        {
