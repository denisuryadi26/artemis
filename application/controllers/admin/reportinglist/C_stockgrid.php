...
class C_stockgrid extends CI_Controller {
    ...
    public function export()
    {
        ...
        if(!empty($all_client))
                $file_name = "stock_grid_all_client_".date("Ymd")."_".date("His");
            else
                $file_name = "stock_grid_".strtolower(str_replace(' ', '_', $program_name))."_".date("Ymd")."_".date("His");
        ...
        // get data 
        $stockgriddownload     =  $this->db->query($stockgrid)->result_array();
        ...
        fputcsv($file, $header);
        foreach ($stockgriddownload as $row){ 
            fputcsv($file,$row);
        }
        fclose($file); 
        exit;
        // $stockgriddownload  =  $this->db->query($stockgrid)->result();
        ...
        // ->setCellValue('N1', 'Basic Price (IDR)')
        // ->setCellValue('O1', 'Publish Price (IDR)')

        // ;

        // // Miscellaneous glyphs, UTF-8
        // $i=2; foreach($stockgriddownload as $data) {
        ...
        // ->setCellValue('N'.$i, $data->basic_price)
        // ->setCellValue('O'.$i, $data->publish_price);
        // $i++;
        // }
        ...
        // // Rename worksheet
        // $spreadsheet->getActiveSheet()->setTitle('Stock Grid'." ".date("Ymd"));
